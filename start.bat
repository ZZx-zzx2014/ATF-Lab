...
REM
REM 实现说明（两个 Windows 批处理的坑）：
REM   1. 编码：cmd.exe 按系统 ANSI 代码页（简体中文为 GBK/936）
REM      读取 .bat 文件。本文件因此保存为 GBK 而非 UTF-8，
REM      否则中文会被解析成乱码并被当作命令执行。
REM   2. 行尾：必须是 CRLF。LF 会导致 cmd 把多行粘连解析。
REM   另外本脚本避免使用嵌套 if/else 块，统一用 goto 跳转。
REM ============================================================
setlocal
cd /d "%~dp0"
